...
function getEditorText() {
  if (window.editorTabs && typeof window.editorTabs.getCurrentFileContent === 'function') {
    return window.editorTabs.getCurrentFileContent();
  }
...
async function saveWithHandle(handle) {
  ...
  if (window.editorTabs && typeof window.editorTabs.setCurrentFileSaved === 'function') {
    window.editorTabs.setCurrentFileSaved();
  }
}
...
function getSuggestedFilename() {
  // Check the current tab name
  const currentTabName = window.editorTabs && typeof window.editorTabs.getCurrentFileName === 'function' ? window.editorTabs.getCurrentFileName() : null;

  // If the current tab is not the default Untitled, use its name
  if (currentTabName && currentTabName !== 'Untitled.js') return currentTabName;

  // For untitled tabs, don't use remembered filename; use the first line
  const text = getEditorText();
  const firstLine = (text.split(/\r?\n/).find(l => l.trim().length) || '').trim();
...
function downloadFallback(filename) {
  ...
  // Update the tab name after download
  if (window.editorTabs && typeof window.editorTabs.setCurrentFileName === 'function') {
    window.editorTabs.setCurrentFileName(finalName);
  }
}
    ...
    try {
      ...
      if (window.editorTabs && typeof window.editorTabs.setCurrentFileName === 'function') {
        try {
          ...
          window.editorTabs.setCurrentFileName(file.name);
...
export async function handleOpenClick() {
...
// Wire up buttons
// If buttons were present in older layouts, keep compatibility by wiring them when found.
